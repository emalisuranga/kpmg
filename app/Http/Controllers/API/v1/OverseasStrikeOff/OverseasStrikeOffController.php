...
namespace App\Http\Controllers\API\v1\OverseasStrikeOff;
...
class OverseasStrikeOffController extends Controller
{
     use _helper;
      ...
      public function loadData(Request $request){
        ...
        if( !( $moduleStatus === 'OVERSEAS_STRIKE_OFF_PROCESSING' ||  $moduleStatus === 'OVERSEAS_STRIKE_OFF_RESUBMIT' ) ) {
         ...
         $offshoreOverseasId = OffshoreStrike::select('id')
                                ->where('request_id',$request_id)
                                ->where('company_id', $companyId)
                                ->where('strike_type',$this->settings('OVERSEAS_STRIKE_OFF','key')->id)
                                ->first();

        if($request_id && $country && $ceasedDate){
            if($offshoreOverseasId){
                $req = OffshoreStrike::find($offshoreOverseasId->id);
           }
           else {
                $req = new OffshoreStrike;
                $req->company_id = $companyId;
                $req->request_id = $request_id;
                $req->strike_type = $this->settings('OVERSEAS_STRIKE_OFF','key')->id;
           }
     ...
     private function valid_request_operation($company_id){
            ...
            $accepted_request_statuses = array(
                $this->settings('OVERSEAS_STRIKE_OFF_APPROVED','key')->id,
                $this->settings('OVERSEAS_STRIKE_OFF_REJECTED','key')->id
            );
            $request_type =  $this->settings('OVERSEAS_STRIKE_OFF','key')->id;
            ...
            if( $exist_request_id){
                ...
                if($request_count !== 1) { // request not in processing or  resubmit stage
                    ...
                    $request->status = $this->settings('OVERSEAS_STRIKE_OFF_PROCESSING','key')->id;
    ...
    function files_for_upload_docs($company_id){
        ...
        // documents list
        $form_prospectus = Documents::where('key', 'FORM_38')->first();
        $form_other_docs = Documents::where('key', 'OVERSEAS_STRIKE_OTHER_DOCUMENTS')->first();
        $external_comment_type_id = $this->settings('COMMENT_EXTERNAL','key')->id;
        ...
        if($request->status == 'OVERSEAS_STRIKE_OFF_RESUBMIT' && isset($uploadeDocStatus) && $uploadeDocStatus ){
        ...
        $regChargeGroup = DocumentsGroup::where('request_type', 'OVERSEAS_STRIKE_OFF_DOCUMENT_GROUP')->first();
    ...
    function files_for_other_docs($company_id){
        ...
        foreach($other_docs as $docs ) {
            ...
            if($uploadeDocStatus == $this->settings('OVERSEAS_STRIKE_OFF_RESUBMIT','key')->id ) { //if doc is resubmitted
    ...
    function upload(Request $request){
            ...
            $path = 'company/overseas/'.$company_id;
    ...
    public function generate_App_pdf(Request $request){
       ...
       'comName' => $company->name,
        ...
        'singning_party_name' => $party_name->first_name.' '.$party_name->last_name,
         ...
         $pdf = PDF::loadView('forms/form38',$fieldset);
        $pdf->stream('form-38.pdf');
    ...
    function removeDoc(Request $request){
     ...
     function resubmit(Request $request ) {
        ...
        $update2 =  CompanyChangeRequestItem::where('id', $request_id)->update(['status' => $this->settings('OVERSEAS_STRIKE_OFF_RESUBMITTED', 'key')->id]);
