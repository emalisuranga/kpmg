...
class OverseasCompanyAlterationsController extends Controller
{
    use _helper;
...
function uploadedDocsWithNoOfPages($companyId){
  ...
  // if($changes_director) {
   ...
   function files_for_upload_docs($company_id){
        ...
        $annual_return_request_type =  $this->settings('OVERSEAS_ALTERATIONS','key')->id;
        ...
        $file_row['file_name'] = $form_35->name;
        $file_row['file_key'] = $form_35->key;
        $file_row['file_type'] = '';
        $file_row['dbid'] = $form_35->id;
        $file_row['file_description'] = $form_35->description;
      ...
      /******** */
        ...
        $form_45 = Documents::where('key', 'OVERSEAS_FORM45')->first();
        $form_46 = Documents::where('key', 'OVERSEAS_FORM46')->first();
      ...
      // if($changes_sec || $changes_sec_firms ) {
        ...
        if(isset($changes[0]->id)){
         ...
         foreach($changes as $member ) {
                ...
                $file_row['doc_status'] = 'DOCUMENT_PENDING';
                $file_row['is_required'] = true;
                $file_row['file_name'] = $form_45->name.' for '.$memberInfo->first_name.' '.$memberInfo->last_name;
                $file_row['file_type'] = '';
                $file_row['dbid'] = $form_45->id;
                $file_row['file_description'] = $form_45->description;
                $file_row['applicant_item_id'] = null;
                $file_row['member_id'] = $memberInfo->id;
                $file_row['firm_id'] = null;
                $file_row['request_id'] = $request_id;
                $file_row['uploaded_path'] = '';
                $file_row['is_admin_requested'] = false;
                ...
                $uploadedDoc =  CompanyDocuments::where('company_id', $company_id)
                                                ->where('request_id',$request_id)
                                                ->where('document_id', $form_45->id )
                                                ->where('company_member_id', $memberInfo->id )
                                                ->orderBy('id', 'DESC')
                                                ->first();
                $uploadeDocStatus = @$uploadedDoc->status;
                $file_row['doc_status'] = isset($this->settings($uploadeDocStatus,'id')->key) ? $this->settings($uploadeDocStatus,'id')->key : 'DOCUMENT_PENDING';
           ...
           //check new power of attorny member
            foreach($changes as $member ) {
                ...
                $file_row['doc_status'] = 'DOCUMENT_PENDING';
                $file_row['is_required'] = true;
                $file_row['file_name'] = $form_46->name.' for '.$memberInfo->first_name.' '.$memberInfo->last_name;
                $file_row['file_type'] = '';
                $file_row['dbid'] = $form_46->id;
                $file_row['file_description'] = $form_46->description;
                $file_row['applicant_item_id'] = null;
       ...
       /* $changes = CompanyItemChange::where('request_id',$request_id)
        ...
        if(isset($changes[0]->id)){
            ...
            foreach($changes as $firm ) {
                ...
                $file_row['doc_status'] = 'DOCUMENT_PENDING';
                $file_row['is_required'] = true;
                $file_row['file_name'] = $form_46->name.' for '.$memberInfo->name;
                $file_row['file_type'] = '';
                $file_row['dbid'] = $form_46->id;
                $file_row['file_description'] = $form_46->description;
                $file_row['applicant_item_id'] = null;
                $file_row['member_id'] = null;
                $file_row['firm_id'] = $memberInfo->id;
                $file_row['request_id'] = $request_id;
                $file_row['uploaded_path'] = '';
                $file_row['is_admin_requested'] = false;
                ...
                $uploadedDoc =  CompanyDocuments::where('company_id', $company_id)
                                                ->where('request_id',$request_id)
                                                ->where('document_id', $form_46->id )
                                                ->where('company_firm_id', $memberInfo->id )
                                                ->orderBy('id', 'DESC')
                                                ->first();
                $uploadeDocStatus = @$uploadedDoc->status;
                $file_row['doc_status'] = isset($this->settings($uploadeDocStatus,'id')->key) ? $this->settings($uploadeDocStatus,'id')->key : 'DOCUMENT_PENDING';
        ...
        $annualReturnGroup = DocumentsGroup::where('request_type', 'OVERSEAS_ALTERATIONS')->first();
        ...
        $form_35_charter = Documents::where('key', 'FORM_35_CHARTER')->first();
        $form_35_statute = Documents::where('key', 'FORM_35_STATUTE')->first();
        $form_35_memo = Documents::where('key', 'FORM_35_MEMORANDUM')->first();
        $form_35_articls = Documents::where('key', 'FORM_35_ARTICLES')->first();
        $form_other_docs = Documents::where('key', 'ALTERATIONS_OF_OVERSEAS_OTHER_DOCUMENTS')->first();
        ...
        if($annualReturnDocumentsCount){
            foreach($annualReturnDocuments as $other_doc ) {
                ...
                if(
                   $form_35->id === $other_doc->id ||
                   $form_45->id === $other_doc->id ||
                   $form_46->id === $other_doc->id ||
                   $form_35_charter->id === $other_doc->id ||
                   $form_35_memo->id === $other_doc->id ||
                   $form_35_articls->id === $other_doc->id ||
                   $form_35_statute->id === $other_doc->id ||
    ...
    function files_for_additional_docs($company_id){
        ...
        foreach($other_docs as $docs ) {
            ...
            $file_row['doc_status'] = isset($this->settings($uploadeDocStatus,'id')->key) ? $this->settings($uploadeDocStatus,'id')->key : 'DOCUMENT_PENDING';
            if($company_status == 'OVERSEAS_ALTERATIONS_RESUBMIT' && isset($uploadeDocStatus) && $uploadeDocStatus ){
                            $file_row['doc_status'] = $this->settings($uploadeDocStatus,'id')->key;
            }
            if($uploadeDocStatus == $this->settings('DOCUMENT_REQUEST_TO_RESUBMIT','key')->id ) { //if doc is resubmitted
    ...
    function files_for_upload_other_docs($company_id){
        ...
        $record = OverseasAlteration::where('request_id', $request_id)->first();
    ...
    public function loadData(Request $request){
       ...
       /* if($loginUserInfo->id  != $company_info->created_by ) {
        ...
        $alterRecord = OverseasAlteration::where('company_id', $request->companyId)
        ->where('request_id', $request_id)
        ->first();
       ...
       $sec_sh_comes_from_director = false;
        ...
        if($director_list_count){
            ...
            foreach($director_list as $director){
                ...
                if($has_remove_record && in_array('FOREIGN_DIRECTOR_CHANGES', $alterTypes)){
                    continue;
                }
                ...
                $has_edit_record = CompanyItemChange::where('request_id',$request_id)
                    ...
                    ->count();
                if($has_edit_record && in_array('FOREIGN_DIRECTOR_CHANGES', $alterTypes)){
                        continue;
                }
       ...
       $changed_director_list_count = CompanyMember::where('company_id',$request->companyId)
                                       ->where('designation_type',$this->settings('DERECTOR','key')->id)
                                       ->where('status',$this->settings('OVERSEAS_ALTERATIONS','key')->id)
                                       ->count();
        ...
        if($sec_firm_list_count){
            ...
            foreach($sec_list as $sec){
                ...
                if($has_remove_record && in_array('FOREIGN_POWER_OF_ATTORNEY_CHANGES', $alterTypes)){
                    continue;
                }
                ...
                $has_edit_record = CompanyItemChange::where('request_id',$request_id)
                    ...
                    ->count();
                if($has_edit_record && in_array('FOREIGN_POWER_OF_ATTORNEY_CHANGES', $alterTypes)){
                        continue;
                }
       ...
       return response()->json([
            ...
            'data'   => array(
              ...
              //  'other_doc_change_date' => $alterRecord->other_doc_change_date,
                ...
                'form15_payment' => $this->settings('PAYMENT_FORM35','key')->value,
                'form45_payment' => $this->settings('PAYMENT_OVERSEAS_FORM45','key')->value,
                'form46_payment' => $this->settings('PAYMENT_OVERSEAS_FORM46','key')->value,
    ...
    private function getPanaltyCharge( $company_id , $request_id ) {
        ...
        $members_date_of_appointments = CompanyMember::leftJoin('company_item_changes', 'company_members.id', '=', 'company_item_changes.item_id')
                ...
                ->where('company_members.status', $this->settings('OVERSEAS_ALTERATIONS', 'key')->id)
        ...
        $firms_date_of_appointments = CompanyFirms::leftJoin('company_item_changes', 'company_member_firms.id', '=', 'company_item_changes.item_id')
                ...
                ->where('company_member_firms.status', $this->settings('OVERSEAS_ALTERATIONS', 'key')->id)
                ->where('company_member_firms.type_id', $this->settings('SECRETARY', 'key')->id)
                ->where('company_item_changes.request_id', $request_id)
                ->pluck('company_item_changes.change_or_effective_date')->toArray();
        ...
        $alterRecord = OverseasAlteration::where('company_id', $company_id)
        ->where('request_id', $request_id)
        ->first();
        ...
        $init_panalty = floatval( $this->settings('PAYMENT_PENALTY_FORM_35_INITIAL','key')->value );
        $increment_penalty = floatval( $this->settings('PAYMENT_PENALTY_FORM_35_INCREMENT','key')->value );
        $max_penalty = floatval( $this->settings('PAYMENT_PENALTY_FORM_35_MAX','key')->value );
    ...
    function updateExistingDirector( Request $request) {
        ...
        if($has_edit_director_record){
            ...
            $newDirector->status =  $this->settings('OVERSEAS_ALTERATIONS','key')->id;
            $newDirector->save();
    ...
    function removeExistingDirector(Request $request ){
       ...
       // $change->change_or_effective_date = date('Y-m-d', time());
    ...
    function updateExistingSec( Request $request) {
        ...
        if($has_edit_sec_record){
            ...
            $newSec->status = $this->settings('OVERSEAS_ALTERATIONS','key')->id;
    ...
    public function updateOtherDocsChangeDate(Request $request){
        ...
        $update = OverseasAlteration::where('request_id', $request_id)->update($update_rec);
    ...
    public function updateAlterationType(Request $request){
        ...
        $alteration = OverseasAlteration::where('request_id', $request_id)->first();
    ...
    private function valid_annual_return_request_operation($company_id){
        ...
        if($exist_request_id) {
            ...
            if($request_count !== 1) { // request not in processing or  resubmit stage
                ...
                $request->status = $this->settings('OVERSEAS_ALTERATIONS_PROCESSING','key')->id;
                $request->request_by = isset($company_info->created_by) ? $company_info->created_by : 1 ;
                $request->save();
                $request_id =  $request->id;

                $record = new OverseasAlteration;
                $record->company_id = $company_id;
                $record->date_of = date('Y-m-d',time());
                $record->request_id = $request_id;
                $record->status = $this->settings('OVERSEAS_ALTERATIONS_PROCESSING','key')->id;
                $record->save();
                $record_id =  $record->id;
    ...
    private function has_annual_record_for_this_year($company_id) {
        ...
        $record_count = OverseasAlteration::where('company_id', $company_id)
                                  ->whereNotIn('status', $accepted_request_statuses )
                                   ->count();
        if( $record_count === 1 ) {
            $record = OverseasAlteration::where('company_id', $company_id)
            ->whereNotIn('status', $accepted_request_statuses )
             ->first();
    ...
    function resubmit(Request $request ) {
        ...
        $annualReturnRecord =  OverseasAlteration::where('company_id', $request->companyId)
            ->where('request_id', $request_id)
             ->first();
        if( !( isset($annualReturnRecord->status) && $annualReturnRecord->status === $this->settings('OVERSEAS_ALTERATIONS_RESUBMIT', 'key')->id)){
            return response()->json([
        ...
        $update1 = OverseasAlteration::where('request_id', $request_id)->update(['status' => $this->settings('OVERSEAS_ALTERATIONS_RESUBMITTED', 'key')->id]);
        $update2 =  CompanyChangeRequestItem::where('id', $request_id)->update(['status' => $this->settings('OVERSEAS_ALTERATIONS_RESUBMITTED', 'key')->id]);
...
private function stakeholder_store($company_id){
    ...
    function upload(Request $request){
            ...
            $path = 'overseas-of-alterations/'.substr($company_id,0,2).'/'.$request_id;
    ...
    function removeDoc(Request $request){
           ...
           if($member_type == 'member') {
            ...
            ->where('company_member_id', $member_id)
            ->delete();
           }
           else if($member_type == 'firm') {
            CompanyDocuments::where('company_id', $company_id)
            ->where('request_id',$request_id)
            ->where('document_id', $file_type_id)
            ->where('company_firm_id', $member_id)
            ->delete();
           } else {
            CompanyDocuments::where('company_id', $company_id)
            ->where('request_id',$request_id)
    ...
    function uploadOtherDocs(Request $request){
        ...
        $path = 'overseas-of-alterations/other-docs/'.substr($company_id,0,2);
         $path=  $request->file('uploadFile')->storeAs($path,$file_name,'sftp');
