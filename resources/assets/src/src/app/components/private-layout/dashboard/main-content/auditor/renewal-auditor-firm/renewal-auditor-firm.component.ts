...
@Component({
  selector: 'app-renewal-auditor-firm',
  templateUrl: './renewal-auditor-firm.component.html',
  styleUrls: ['./renewal-auditor-firm.component.scss']
})
export class RenewalAuditorFirmComponent implements OnInit {
  ...
  auditorGeneratePDF(token) {
    this.spinner.show();
    this.auditorService.auditorFirmRenewalPDF(token)
      .subscribe(
        response => {
          this.spinner.hide();
  ...
  checkRegNum() {
    ...
    this.auditorService.auditorFirmIsReg(data)
      .subscribe(
        req => {
          if (req['status']) {
            ...
            alert('Invalid token or unregistered audit firm');
            this.router.navigate(['/dashboard/home']);
          }
        },
        error => {
  ...
  getCipherToken() {
    ...
    const item: Array<Item> = [{
      fee_type: 'PAYMENT_RENEWAL_FIRM_AS_AUDITORS',
      description: 'Auditor firm renewal',
      quantity: 1,
    }];
    const buy: IBuy = {
      module_type: 'MODULE_AUDITOR_RENEWAL_FIRM',
