...
@Component({
  selector: 'app-records-registers-resubmit',
  templateUrl: './records-registers-resubmit.component.html',
  styleUrls: ['./records-registers-resubmit.component.scss']
})
export class RecordsRegistersResubmitComponent implements OnInit, AfterViewInit {
  ...
  fileUploadUpdate(event, id, description, docType) {
    ...
    if (fileList.length > 0) {
      ...
      let uploadurl = this.url.getrrFileUpdateUploadUrl();
  ...
  fileDeleteUpdate(docId, docType, index) {
    ...
    this.rrAddressChangeService.rrDeleteUploadedUpdatePdf(data)
      .subscribe(
        response => {
  ...
  rrReSubmit() {
    ...
    this.rrAddressChangeService.rrReSubmit(data)
      .subscribe(
        req => {

          localStorage.removeItem('RRstorage');
      ...
      .subscribe(
        req => {
          ...
          if (req['data']['oldrecaddresses']){
            ...
            // setTimeout(this.validateCourtSection, 3000);
          }
  ...
  addPresidentDataToArray() {
    const data = {
      ...
      discription: this.address['discription'],
    };
  ...
  addShareDataToArray() {
    const data = {
      ...
      discription: this.saddress['discription'],
    };
  ...
  addMemberDataToArray() {
    const data = {
      ...
      discription: this.maddress['discription'],
    };
