...
checkDate2() {
      ...
      .subscribe(
        req => {
          if (req['data']) {
            ...
            this.charges = req['data']['charges'];
            this.chargesWithDeeds = req['data']['chargesWithDeeds'];
  ...
  fileDelete(docId, docType, index) {
    if (confirm('Are you sure you want to delete this document?')) {
      ...
      this.spinner.show();
      this.memoService.scDeleteUploadedPdf(data)
        .subscribe(
          response => {
  ...
  getCipherToken() {
    ...
    const item: Array<Item> = [{
      fee_type: 'PAYMENT_COMPANY_SATISFACTION_CHARGE_CHANGE',
      description: `For Company Satisfaction Charge - ${this.companyRegno}`,
      quantity: 1,
    }];

    const buy: IBuy = {
      module_type: 'MODULE_COMPANY_SATISFACTION_CHARGE_CHANGE',
      module_id: this.requestId,
      description: 'Company Satisfaction Charge',
      item: item,
      extraPay: null
    };
