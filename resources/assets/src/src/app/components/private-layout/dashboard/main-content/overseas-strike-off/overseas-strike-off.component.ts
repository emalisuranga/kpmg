...
import { OverseasStrikeOffService } from './overseas-strike-off.service';
import { ApiOverseasStrikeOffConnection } from './ApiOverseasStrikeOffConnection';

@Component({
  selector: 'app-overseas-strike-off',
  templateUrl: './overseas-strike-off.component.html',
  styleUrls: ['./overseas-strike-off.component.scss']
})
export class OverseasStrikeOffComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    ...
    private OverseasStrikeOffService: OverseasStrikeOffService
  ) {
  ...
  loadCompanyData() {
    ...
    this.spinner.show();
    // console.log(data);
    this.OverseasStrikeOffService.loadCompanyAddress(data).subscribe(
      req => {
        this.moduleStatus = req['data']['moduleStatus'];

        if (
          !(
            this.moduleStatus === 'OVERSEAS_STRIKE_OFF_PROCESSING' ||
            this.moduleStatus === 'OVERSEAS_STRIKE_OFF_RESUBMIT'
          )
        ) {
          this.spinner.hide();
  ...
  removeDoc(docTypeId) {
    ...
    this.spinner.show();
    this.OverseasStrikeOffService.removeDoc(data)
      .subscribe(
        rq => {
          this.loadCompanyData();
          //   this.spinner.hide();
  ...
  removeOtherDoc(token) {
    ...
    this.OverseasStrikeOffService.removeOtherDoc(data)
      .subscribe(
        req => {
          this.loadCompanyData();
        }
  ...
  pay() {
    ...
    this.paymentItems.push(
      {
        fee_type: 'PAYMENT_OVERSEAS_STRIKE_OFF',
        description: 'Overseas Strike Off Payment',
        quantity: 1,
      }
    ...
    const buy: IBuy = {
      module_type: 'OVERSEAS_STRIKE_OFF',
      module_id: this.requestId.toString(),
      description: 'Overseas Strike Off accounts',
  ...
  resubmit() {
    ...
    this.OverseasStrikeOffService.resubmit(data)
      .subscribe(
        req => {
          // this.loadData();
          if (req['status']) {
  ...
  affairsGenerateDOC() {
    ...
    this.OverseasStrikeOffService.getApplicationPDFService(data).subscribe(
      response => {
        console.log('test');
        this.helper.download(response);
      },
